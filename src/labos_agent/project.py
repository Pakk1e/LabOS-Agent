...
def build_continuation_prompt(snapshot: ProjectSnapshot, message: str, ci_feedback: str|None = None) -> str:
    ...
    if ci_feedback:
        parts += ["","Latest LabOS local CI result:",ci_feedback]
    ...
    if ci_feedback:
        parts += ["","LocalCI gate: the controller has run the configured local CI stage against the current working tree. If it passed, use the next iteration to commit/push the already-validated changes. If it failed, fix the reported failure and leave the changes uncommitted for another LocalCI run.","","Pre-push rule: when a local CI stage is configured, do not commit or push changes during the implementation iteration that produced them. Leave source changes in the working tree so LabOS can validate them locally before a later commit/push iteration."]
    return "\n".join(parts)
