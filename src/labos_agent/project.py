...
def build_continuation_prompt(snapshot: ProjectSnapshot, message: str, ci_feedback: str|None = None) -> str:
    ...
    if ci_feedback:
        parts += ["","LocalCI handoff: the result above belongs to the previous controller iteration. If it passed, treat the previously reported working-tree changes as locally validated and commit/push those changes before starting unrelated implementation work. If it failed, fix that failure and leave the changes uncommitted for another LocalCI run.","","Pre-push rule: when a local CI stage is configured, do not commit or push changes during the implementation iteration that produced them. Leave source changes in the working tree so LabOS can validate them locally before a later commit/push iteration."]
    return "\n".join(parts)
